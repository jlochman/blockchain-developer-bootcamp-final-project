...
contract("MockNFT", function (accounts) {
  ...
  before(async () => {
    nftContract = await MockNFT.deployed();
  });
  
  it("there are no tokens", async function () {
    assert.equal(await nftContract.balanceOf(owner), 0, `tokenBalance of owner should be 0`);
    assert.equal(await nftContract.balanceOf(alice), 0, `tokenBalance of alice should be 0`);
    assert.equal(await nftContract.balanceOf(bob), 0, `tokenBalance of bob should be 0`);
  });
  ...
  describe("Mint tokens", () => {
    ...
    it ("owner should mint token for himself", async () => {
        ...
        const tokenBalance = await nftContract.balanceOf(owner);
    ...
    it ("owner should mint tokens for others", async () => {
      await nftContract.safeMint(alice);
      await nftContract.safeMint(bob);

      const tokenBalanceAlice = await nftContract.balanceOf(alice);
      assert.equal(tokenBalanceAlice, 1, `tokenBalance of Alice should be 1 but is ${tokenBalanceAlice}`);
      const tokenBalanceBob = await nftContract.balanceOf(bob);
    ...
    it ("non-owner shouldn't be able to mint tokens", async () => {
      try {
        await nftContract.safeMint(alice, {from: alice});
      } catch (err) {}
      const tokenBalance = await nftContract.balanceOf(alice);
  ...
  describe("Set Roaylties", () => {
    ...
    it ("royalty cannot be bigger than 5%", async () => {
      try {
        await nftContract.setRoyalties([owner, alice], [500, 100]);
      } catch (err) {}
      const ownerRoyalty = await nftContract.getRoyalty(owner);
      assert.equal(ownerRoyalty, 100, `royalty of owner should be 100 but is ${ownerRoyalty}`);
    });
    ...
    it ("royalty cannot be set for more than 5 accounts", async () => {
      try {
        await nftContract.setRoyalties([owner, alice, bob, accounts[3], accounts[4], accounts[5]], [500, 50, 50, 50, 50, 50]);
      } catch (err) {}
      const ownerRoyalty = await nftContract.getRoyalty(owner);
      assert.equal(ownerRoyalty, 100, `royalty of owner should be 100 but is ${ownerRoyalty}`);
    });
    ...
    it ("input data missmatch", async () => {
      try {
        await nftContract.setRoyalties([owner, alice], [500, 50, 50]);
      } catch (err) {}
      const ownerRoyalty = await nftContract.getRoyalty(owner);
      assert.equal(ownerRoyalty, 100, `royalty of owner should be 100 but is ${ownerRoyalty}`);
    });
    ...
    it ("owner should update royalties", async () => {
      await nftContract.setRoyalties([owner, bob], [200, 100]);
  ...
  describe("Modify Offers", () => {
    ...
    it ("there are no offers", async () => {
      assert.equal(await nftContract.getOfferPrice(0), 0, `price for tokenId=0 should be 0`);
      assert.equal(await nftContract.getOfferPrice(1), 0, `price for tokenId=1 should be 0`);
      assert.equal(await nftContract.getOfferPrice(2), 0, `price for tokenId=2 should be 0`);
    });

    it ("token owner can create an offer", async () => {
      await nftContract.setOfferPrice(0, 10000);

      const token0OfferPrice = await nftContract.getOfferPrice(0);
    ...
    it ("token owner can modify an offer", async () => {
      await nftContract.setOfferPrice(0, 15000);
    ...
    it ("only token owner can create an offer", async () => {
      try {
        await nftContract.setOfferPrice(0, 5000, {from: alice});
      } catch (err) {}
      const token0OfferPrice = await nftContract.getOfferPrice(0);
      assert.equal(token0OfferPrice, 15000, `token0OfferPrice should be 15000 but is ${token0OfferPrice}`);
    });
    ...
    it ("only token owner can cancel an offer", async () => {
      try {
        await nftContract.cancelOffer(0, {from: alice});
      } catch (err) {}
      const token0OfferPrice = await nftContract.getOfferPrice(0);
      assert.equal(token0OfferPrice, 15000, `token0OfferPrice should be 15000 but is ${token0OfferPrice}`);
    });
    ...
    it ("token owner can cancel an offer", async () => {
      await nftContract.cancelOffer(0);
  ...
  describe("Trade Tokens", () => {
    ...
    it ("token transfered", async () => {
      assert.equal(await nftContract.balanceOf(owner), 1, `tokenBalance of owner should be 1`);
      assert.equal(await nftContract.balanceOf(alice), 1, `tokenBalance of Alice should be 1`);
      
      await nftContract.setOfferPrice(0, 10000);
      await nftContract.acceptOffer(0, {from: alice, value: 10000});
      assert.equal(await nftContract.balanceOf(owner), 0, `tokenBalance of owner should be 0`);
      assert.equal(await nftContract.balanceOf(alice), 2, `tokenBalance of Alice should be 2`);
    });


    it ("offer cancelled", async () => {
      await nftContract.setOfferPrice(0, 10000, {from: alice});
      assert.equal(await nftContract.getOfferPrice(0), 10000, `offerPrice for token=0 should be 10000`);

      await nftContract.acceptOffer(0, {value: 10000});
      assert.equal(await nftContract.getOfferPrice(0), 0, `offerPrice for token=0 should be 0`);
    });
    ...
    it ("payments distributed", async () => {
      ...
      const receipt = await nftContract.acceptOffer(0, {from: alice, value: 10000});
    ...
    it ("payment for non-existing order does nothing", async () => {
      try {
        await nftContract.acceptOffer(5, {from: accounts[5], value: 1e18});
      } catch (err) {}
      const tokensOwned = await nftContract.balanceOf(accounts[5]);
    ...
    it ("insufficient payment does nothing", async () => {
      await nftContract.setOfferPrice(0, 10000, {from: alice});
      try {
        await nftContract.acceptOffer(0, {value: 8000});
      } catch (err) {}
      const tokensOwned = await nftContract.balanceOf(owner);
